...
export default function CreatePost() {
  ...
  // ── Settings Panel Content (shared between sidebar & mobile sheet) ──
      ...
      <div className="sticky top-0 z-40 h-12 sm:h-14 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 px-3 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            onClick={handleSaveDraft}
            disabled={savedStatus === "saving"}
            className="sm:hidden p-2 rounded-xl text-white/60 hover:bg-white/10 border border-white/10 transition disabled:opacity-50"
          >
      ...
      <div className="flex max-w-[1440px] mx-auto min-h-[calc(100vh-3.5rem)]">
        ...
        <div className="flex-1 min-w-0 px-3 sm:px-6 py-4 sm:py-8 lg:px-12">
          ...
          <div className="prose prose-invert max-w-none">
            <AdvancedEditor onChange={setEditorContent} />
          </div>
        </div>

        {/* ── RIGHT: Desktop Settings Sidebar ── */}
          ...
          <div className="sticky top-20 mr-4 xl:mr-10 space-y-6">
            ...
            <div className="bg-white/2 backdrop-blur-md rounded-2xl border border-white/5 px-5 py-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-xs text-white/30 uppercase tracking-widest font-bold">Reading Time</span>
                <span className="text-xs text-white/60 font-medium">
                  {editorContent
                    ? Math.ceil(JSON.stringify(editorContent).split(/\s+/).length / 200)
                    : 0} min
                </span>
              </div>
            </div>
